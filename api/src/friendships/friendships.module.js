"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
exports.__esModule = true;
exports.FriendshipsModule = void 0;
var common_1 = require("@nestjs/common");
var typeorm_1 = require("@nestjs/typeorm");
var user_entity_1 = require("../user/user.entity");
var user_module_1 = require("../user/user.module");
var friendships_controller_1 = require("./friendships.controller");
var friendships_service_1 = require("./friendships.service");
var frienship_entity_1 = require("./frienship.entity");
var FriendshipsModule = /** @class */ (function () {
    function FriendshipsModule() {
    }
    FriendshipsModule = __decorate([
        (0, common_1.Module)({
            controllers: [friendships_controller_1.FriendshipsController],
            providers: [friendships_service_1.FriendshipsService],
            exports: [friendships_service_1.FriendshipsService],
            imports: [typeorm_1.TypeOrmModule.forFeature([frienship_entity_1.Friendships, user_entity_1.User]), user_module_1.UserModule]
        })
    ], FriendshipsModule);
    return FriendshipsModule;
}());
exports.FriendshipsModule = FriendshipsModule;
